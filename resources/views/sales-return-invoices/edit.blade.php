...
<div class="container">
    ...
    <form action="{{ route('sales-return-invoices.update', $invoice->id) }}" method="POST" class="col-md-12" autocomplete="off">
        @csrf
        @method('PUT')
        <div class="row">
            <!-- Customer Phone -->
            ...
            <div class="mb-3 col-md-4">
                ...
                <input type="text" class="form-control" name="against_invoice_no" id="salesInvoiceInput"
                       value="{{ old('against_invoice_no', $invoice->against_invoice_no) }}">
            </div>
                ...
                <select class="form-select" name="payment_method">
                    @foreach($paymentMethods as $method)
                        <option value="{{ $method->id }}"
                            {{ $invoice->payment_method == $method->id ? 'selected' : '' }}>
                            {{ $method->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <table class="table table-bordered" id="itemTable">
            ...
            <tbody>
            @foreach($invoice->salesReturnItem as $index => $item)
                <tr>
                    <td><input type="text" name="items[{{ $index }}][barcode]" class="form-control barcode"
        ...
        <div class="mt-3">
            ...
            <a class="btn btn-danger col-md-2 btn-sm" href='/sales-return-invoices'>Cancel</a>
        </div>
    </form>
</div>
