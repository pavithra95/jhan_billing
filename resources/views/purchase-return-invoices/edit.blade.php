@extends('layout.master') @section('title', 'ZetBooks') @section('content_header') @stop @section('content')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <form ref="form" action="/{{$url}}/{{$sales->id}}" method="POST" role="form" class="col-md-12" autocomplete="off">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group @if($errors->has('customer_id')) text-danger @endif">
                                ...
                                <!--<select v-model="selectedCustomer" name="vendor_id" id="inputCustomer_id"@input="refreshCustomer" class="form-control" required="required">-->
                                <!--    <option :value="customer.id"  v-for="customer in customers">@{{customer.name}}</option>-->
                                <!--</select>-->
                                <v-select :options="customers" label="name" v-model="selectedCustomer" @input="refreshCustomer"> </v-select>
                                <template v-if="selectedCustomer != null">
                                    <input type="hidden" name="vendor_id" :value="selectedCustomer.id" />
                                </template>
                                {{--
                        ...
                        <div class="col-md-4">
                            <div class="form-group @if($errors->has('invoice_no')) text-danger @endif">
                                ...
                                <input type="text" name="invoice_no" class="form-control" id="invoice_no" required="required" value="{{$sales->invoice_no}}" disabled="" />
                        ...
                        <div class="col-md-4">
                            <div class="form-group @if($errors->has('invoice_date')) text-danger @endif">
                                ...
                                <input type="text" name="invoice_date" class="form-control date-picker" required="" value="{{$sales->invoice_date}}" />
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group @if($errors->has('due_date')) text-danger @endif">
                                ...
                                <input type="text" name="due_date" class="form-control date-picker" value="{{$sales->due_date}}" />
                        ...
                        <div class="col-md-4">
                            <div class="form-group @if($errors->has('due_date')) text-danger @endif">
                                ...
                                <select name="payment_method_id" id="inputPayment_method_id" class="form-control" required="required">
                                    @foreach ($payment as $item) {{-- expr --}}
                                    <option @if ($sales->payment_method_id == $item->id ) selected="" @endif value="{{$item->id}}">{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group @if($errors->has('due_date')) text-danger @endif">
                                ...
                                <textarea name="notes" id="" class="form-control">{{$sales->notes}}</textarea>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            <tr v-for="(item, index) in invoice_items">
                                                    ...
                                                    <template v-if="checkCustomerGst">
                                                        <v-select :options="items" label="name" v-model="item.item_id" @input="setItem(index, item)" :reduce="item => item.id">
                                                            <template #search="{attributes, events}">
                                                                <input class="vs__search" :required="!item.item_id" v-bind="attributes" v-on="events" />
                                                            </template>
                                                        </v-select>
                                                ...
                                                <td style="width: 150px;"><input class="form-control amount" type="number" :min="1" v-model="item.quantity" @input="setRecalculatedamount(index)" /></td>
                                                ...
                                                <template v-if="selectedCustomer != null">
                                                    <template v-if="checkCustomerGst">
                                                        <td>
                                                            <v-select :options="taxes" label="g_name" v-model="item.group_type_name" @input="setRecalculatedamount(index)"> </v-select>
                                                        </td>
                                                        <td>
                                                            <v-select :options="cess_taxes" label="c_name" v-model="item.cess_group_type_name" @input="setRecalculatedamount(index)"> </v-select>
                                                        </td>
                                                    </template>
                                                    <template v-else>
                                                        <td>
                                                            <v-select :options="taxes_out" label="g_name" v-model="item.group_type_name" @input="setRecalculatedamount(index)"> </v-select>
                                                        </td>
                                                        <td>
                                                            <v-select :options="cess_taxes_out" label="c_name" v-model="item.cess_group_type_name" @input="setRecalculatedamount(index)"> </v-select>
                                                        </td>
                                                    </template>
                                                </template>
                                            ...
                                            <tr>
                                                <td><a v-if="!isError && invoice_items[invoice_items.length-1].price > 0 && errors.length == 0" href="" @click.prevent="addItem" class="btn btn-primary">Add Row</a></td>
                    ...
                    <div class="col-md-6 offset-md-5">
                        ...
                        <a class="btn btn-danger btn-sm" href="/sales-invoices">Cancel </a>
                    </div>
                </form>
            </div>
        </div>
...
<script>
        ...
        var app = new Vue({
          ...
          data: {
            ...
            selectedCustomer: {
                id: "{{$sales->vendor_id}}",
                name: "{{$sales->vendor->name}}",
                state_id: "{{$sales->vendor->state_id}}",
            },
            gsts: [],
             paid:"{{$sales->paid_amount}}",
            pay : 2,
     ...
     @foreach ($invoice_items as $item)
    {
        ...
        taxItems: [],
         group_type_name: {
            id: {{$item->Gst[0]->group_id}},
            g_name: "{{$item->Gst[0]->name}}",
            items: [


               @foreach ($item->Gst[0]->GstItem as $element)

            {
                id: {{$element->tax_item_id}},
                name: "{{$element->name}}",
        ...
        cess_group_type_name: {
            ...
            id: {{$item->Cess[0]->group_id}},
            c_name: "{{$item->Cess[0]->name}}",
            items: [


               @foreach ($item->Cess[0]->GstItem as $element)
